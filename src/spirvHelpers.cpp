...
SpvBlob glsl2spv(VkShaderStageFlags stage, const char* shaderSource) 
{
    ...
    GLSLANG_STAGE_MAPPING(VK_SHADER_STAGE_INTERSECTION_BIT_KHR, GLSLANG_STAGE_INTERSECT);
    ...
    SpvBlob spvBlob = {new uint32_t[size], size * sizeof(uint32_t)};

    glslang_program_SPIRV_get(program, spvBlob.first);
